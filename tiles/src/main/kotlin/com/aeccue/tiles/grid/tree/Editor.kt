...
internal fun MutableTileGridLayoutTree.toEditorConstraintSet(
    flipVertical: Boolean,
    flipHorizontal: Boolean,
    spacing: Dp,
    minimumLineTouchSize: Dp
    ...
    root.constrain(
        ...
        gestureMargin = minimumLineTouchSize / 2
    )
}

private fun Node.constrain(
    ...
    scope.apply {
        when (orientation) {
            Orientation.Vertical -> {
                ...
                nodes.forEachIndexed { index, node ->
                    ...
                    anchorCache =
                        if (!flipVerticalActual) childBounds.bottom else childBounds.top
                }
            }
        ...
        lineReference?.let {
            when (orientation) {
                Orientation.Vertical -> {
                    constrainFill(it) {
                        val lineAnchor = if (!flipHorizontalActual) {
                            bounds.left.anchor
                        } else {
                            bounds.right.anchor
                        }

                        top.linkTo(bounds.top.anchor)
                        bottom.linkTo(bounds.bottom.anchor)
                        absoluteLeft.linkTo(lineAnchor)
                        absoluteRight.linkTo(lineAnchor)
                    }
                }
                Orientation.Horizontal -> {
                    constrainFill(it) {
                        val lineAnchor = if (!flipVerticalActual) {
                            bounds.top.anchor
                        } else {
                            bounds.bottom.anchor
                        }

                        top.linkTo(lineAnchor)
                        bottom.linkTo(lineAnchor)
                        absoluteLeft.linkTo(bounds.left.anchor)
                        absoluteRight.linkTo(bounds.right.anchor)
                    }
                }
            }
        }
...
private fun Node.Leaf.constrain(
    ...
    scope.apply {
        ...
        constrainFill(gestureReference) {
            top.linkTo(
                anchor = reference.top,
                margin = if (bounds.top.linked) gestureMargin else 0.dp
            )
            bottom.linkTo(
                anchor = reference.bottom,
                margin = if (bounds.bottom.linked) gestureMargin else 0.dp
            )
            absoluteLeft.linkTo(
                anchor = reference.absoluteLeft,
                margin = if (bounds.left.linked) gestureMargin else 0.dp
            )
            absoluteRight.linkTo(
                anchor = reference.absoluteRight,
                margin = if (bounds.right.linked) gestureMargin else 0.dp
            )
        }
        ...
        lineReference?.let {
            when (orientation) {
                Orientation.Vertical -> {
                    constrainFill(it) {
                        val lineAnchor = if (!canFlipVertical || !flipVertical) {
                            bounds.top.anchor
                        } else {
                            bounds.bottom.anchor
                        }
                ...
                Orientation.Horizontal -> {
                    constrainFill(it) {
                        val lineAnchor = if (!canFlipHorizontal || !flipHorizontal) {
                            bounds.left.anchor
                        } else {
                            bounds.right.anchor
                        }
